...
class Block(JSONable):
    def __init__(self, dict):
        ...
        self.extra_data: str = get_dict_val(dict, 'extraData', 'extra_data')
        self.gas_limit: int = hex_to_int(
            get_dict_val(dict, 'gasLimit', 'gas_limit'))
        ...
        self.mix_hash: str = get_dict_val(dict, 'mixHash', 'mix_hash')
        self.nonce: str = dict.get('nonce')
        self.number: int = hex_to_int(dict.get('number'))
        self.parent_hash: str = get_dict_val(dict, 'parentHash', 'parent_hash')
        self.receipts_root: str = get_dict_val(
            dict, 'receiptsRoot', 'receipts_root')
        self.sha3_uncles: str = get_dict_val(dict, 'sha3Uncles', 'sha3_uncles')
        self.size: int = hex_to_int(dict.get('size'))
        self.state_root: str = get_dict_val(dict, 'stateRoot', 'state_root')
        self.timestamp: int = hex_to_int(dict.get('timestamp'))
        self.total_difficulty: int = hex_to_int(
            get_dict_val(dict, 'totalDifficulty', 'total_difficulty'))
