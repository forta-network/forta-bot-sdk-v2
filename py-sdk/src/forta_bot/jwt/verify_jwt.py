...
def provide_verify_jwt(logger: Logger) -> VerifyJwt:
    ...
    async def verify_jwt(token: str, polygon_url: str = 'https://polygon-rpc.com') -> bool:
        ...
        if recovered_address != scanner_address:
            logger.error(
                f'signature invalid: expected={scanner_address}, got={recovered_address}')
            return False

        w3 = AsyncWeb3(AsyncWeb3(polygon_url))
        contract = w3.eth.contract(
            address=DISPATCH_CONTRACT, abi=DISPATCH_CONTRACT_ABI)
